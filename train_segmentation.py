...
try:
    ...
    warnings.warn(
        f"The python Nvidia management library could not be imported. Ignore if running on CPU only."
    )
...
def verify_weights(num_classes, weights):
    ...
    if num_classes == 1 and len(weights) == 2:
        ...
        raise ValueError(
            "The number of class weights in the configuration file is different than the number of classes"
        )
...
def create_dataloader(samples_folder, batch_size, num_devices, params):
    ...
    assert (
        len([f for f in samples_folder.glob("**/*.hdf5")]) >= 1
    ), f"Couldn't locate .hdf5 files in {samples_folder}"
    ...
    for subset in ["trn", "val", "tst"]:
        datasets.append(
            dataset_constr(
                ...
                geom_transform=aug.compose_transforms(
                    params, subset, type="geometric", ignore_index=dontcare_val
                ),
    ...
    trn_dataloader = DataLoader(
        trn_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True, drop_last=True
    )
    ...
    val_dataloader = DataLoader(
        val_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True, drop_last=True
    )
    tst_dataloader = (
        DataLoader(
            tst_dataset,
    ...
    for i in ["trn", "val", "tst"]:
        if get_key_def(f"num_{i}_samples", params["training"], None) is not None:
            ...
            if num_samples[i] > file_num_samples:
                raise IndexError(
                    f"The number of training samples in the configuration file ({num_samples[i]}) "
...
def set_hyperparameters(params, num_classes, model, checkpoint, dontcare_val):
    ...
    lr = get_key_def(
        "learning_rate", params["training"], None, "missing mandatory learning rate parameter"
    )
    weight_decay = get_key_def(
        "weight_decay", params["training"], None, "missing mandatory weight decay parameter"
    )
    step_size = get_key_def(
        "step_size", params["training"], None, "missing mandatory step size parameter"
    )
    ...
    class_weights = (
        torch.tensor(params["training"]["class_weights"])
    ...
    # Loss function
    criterion = MultiClassCriterion(
        loss_type=params["training"]["loss_fn"], ignore_index=dontcare_val, weight=class_weights
    )

    # Optimizer
    opt_fn = params["training"]["optimizer"]
    optimizer = create_optimizer(
        params=model.parameters(), mode=opt_fn, base_lr=lr, weight_decay=weight_decay
    )
...
def main(params, config_path):
    ...
    num_classes_corrected = (
        num_classes + 1
    )  # + 1 for background # FIXME temporary patch for num_classes problem.

    # INSTANTIATE MODEL AND LOAD CHECKPOINT FROM PATH
    model, checkpoint, model_name = net(
        params, num_classes_corrected
    ...
    min_annot_perc = get_key_def(
        "min_annotated_percent", params["sample"]["sampling_method"], 0, expected_type=int
    )
    ...
    if bucket_name:
        ...
        bucket, bucket_output_path, output_path, data_path = download_s3_files(
            bucket_name=bucket_name, data_path=data_path, output_path=output_path
        )
    ...
    device = torch.device(
        f"cuda:{lst_device_ids[0]}" if torch.cuda.is_available() and lst_device_ids else "cpu"
    )
    print(
    ...
    trn_dataloader, val_dataloader, tst_dataloader = create_dataloader(
        samples_folder=samples_folder, batch_size=batch_size, num_devices=num_devices, params=params
    )
    ...
    model, criterion, optimizer, lr_scheduler = set_hyperparameters(
        params, num_classes_corrected, model, checkpoint, dontcare
    )
    ...
    if vis_batch_range is not None:
        ...
        assert (
            isinstance(vis_batch_range, list)
            and len(vis_batch_range) == 3
            and all(isinstance(x, int) for x in vis_batch_range)
        )
    ...
    for epoch in range(0, params["training"]["num_epochs"]):
        ...
        writer.add_scalar("Loss/train", trn_report["loss"].avg, epoch)
        if params["training"]["batch_metrics"] is not None:
            writer.add_scalar("Loss/val", val_report["loss"].avg, epoch)
            writer.add_scalar("Iou/val", val_report["iou"].avg, epoch)
        writer.add_scalars(
            "Train/loss",
            {"Loss/train": trn_report["loss"].avg, "Loss/val": val_report["loss"].avg},
            epoch,
        )
        ...
        if val_loss < best_loss:
            ...
            if (
                vis_batch_range is not None
                and vis_at_checkpoint
                and epoch - last_vis_epoch >= ep_vis_min_thresh
            ):
                if last_vis_epoch == 0:
                    tqdm.write(
        ...
        if bucket_name:
            save_logs_to_bucket(
                bucket, bucket_output_path, output_path, now, params["training"]["batch_metrics"]
            )
    ...
    if (
        int(params["training"]["num_epochs"]) > 0
    ...
    if params["training"]["batch_metrics"] is not None:
        test_metric_log["tst/precision"] = tst_report["precision"].avg
        test_metric_log["tst/recall"] = tst_report["recall"].avg
        test_metric_log["tst/fscore"] = tst_report["fscore"].avg
        test_metric_log["tst/iou"] = tst_report["iou"].avg
...
def evaluation(
    ...
    with tqdm(
        eval_loader, dynamic_ncols=True, desc=f"Iterating {dataset} batches with {device.type}"
    ) as _tqdm:
        for batch_index, data in enumerate(_tqdm):
            progress_log.open("a", buffering=1).write(
            ...
            with torch.no_grad():
                ...
                if (dataset == "val") and (batch_metrics is not None):
                    ...
                    assert batch_metrics <= len(_tqdm), (
                        f"Batch_metrics ({batch_metrics} is smaller than batch size "
                    ...
                    if (
                        ...
                        eval_metrics = iou(
                            segmentation, labels_flatten, batch_size, num_classes, eval_metrics
                        )
                ...
                _tqdm.set_postfix(
                    OrderedDict(dataset=dataset, loss=f'{eval_metrics["loss"].avg:.4f}')
                )
...
def vis_from_dataloader(
    params, eval_loader, model, ep_num, output_path, dataset="", device=None, vis_batch_range=None
):
    """
    ...
    with tqdm(eval_loader, dynamic_ncols=True) as _tqdm:
        for batch_index, data in enumerate(_tqdm):
            if vis_batch_range is not None and batch_index in range(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "param_file", metavar="DIR", help="Path to training parameters stored in yaml"
    )
