...
The sampling parameters can then be used in [images_to_samples.py](../sampling_segmentation.py) to obtain the desired dataset or can be use alone this way there is no need to run the full code to find out how the classes are distributed (see the [example](#Running)).
...
## **Sampling**
This section is use by [images_to_samples.py](../sampling_segmentation.py) to prepare the images for the training, validation and inference. Those images must be geotiff combine with a GeoPackage, otherwise it will not work.

In addition, [images_to_samples.py](../sampling_segmentation.py) will assert that all geometries for features in GeoPackages are valid according to [Rasterio's algorithm](https://github.com/mapbox/rasterio/blob/d4e13f4ba43d0f686b6f4eaa796562a8a4c7e1ee/rasterio/features.py#L461).
...
#### Running [images_to_samples.py](../sampling_segmentation.py)
...
You must run [`images_to_samples.py`](../sampling_segmentation.py) code before training to generate the `hdf5` use for the training and the validation.
