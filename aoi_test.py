...
class AOI(object):
    ...
    def __init__(
            ...
            for tiff_band, raster in self.raster_parsed.items():
                with rasterio.open(raster, "r") as src:
                    ...
                    for band in bands_count:
                        ...
                        grab_data.append(self.is_low_contrast())
                        self.raster = src if self.raster_src_is_multiband else None
        self.high_or_low_contrast = grab_data
        ...
        self.num_bands = len(self.raster_bands_request)
        if raster_num_bands_expected:
            if not len(self.raster_parsed) == raster_num_bands_expected:
                logging.critical(
    ...
    @staticmethod
    def equalize_adapthist_enhancement2(
        aoi_chunk: np.ndarray,
        clip_limit: int,
    ):
        """This function applies equalize_adapthist on each chunk of dask array
        ...
        if chunk_data.size > 0 and chunk_data is not None:
            try:
                ...
                slices = dask.array.core.slices_from_chunks(
                    dask.array.empty(chunk_data.shape).chunks
                ...
                for slice_ in slices:
                    tensor = torch.as_tensor(chunk_data[slice_][np.newaxis, ...]).to(
                        device
                    )
                    with torch.no_grad():
                        prediction_output[:, slice_[1], slice_[2]] = (
                            model(tensor).cpu().numpy()[0]
                        )
    ...
    def write_inference_to_tiff(
        ...
        first_underscore_index = self.raster_name.stem.find("_")
        extent = "_Clahe25.tif"
        if doEnhancement:
            extent = "_dask__mask_improve.tif"
        # Extract the substring before the first underscore
        mask_path = self.work_dir.joinpath(self.raster_name.stem + extent)
                ...
                **self.raster_meta,
                ...
                for i in range(mask_image.shape[0]):
                    mask_image_ = mask_image[
                        i, : mask_image.shape[1], : mask_image.shape[2]
                    ]
                    print("we are here 1")
                    dest.write(mask_image_, i + 1)
                    logging.info(f"Mask saved to {mask_path}")
