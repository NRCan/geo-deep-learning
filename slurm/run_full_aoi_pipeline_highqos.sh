...
#   WORKFLOW=inference sbatch slurm/run_full_aoi_pipeline_highqos.sh /path/to/aoi_folder
...
#SBATCH --qos=nrcan__gpu_a100_gpsc7_allocation

set -euo pipefail

if [[ $# -ne 1 ]]; then
    echo "Usage: sbatch slurm/run_full_aoi_pipeline_highqos.sh /path/to/aoi_folder" >&2
    echo "       WORKFLOW=inference sbatch slurm/run_full_aoi_pipeline_highqos.sh /path/to/aoi_folder" >&2
    exit 1
fi
