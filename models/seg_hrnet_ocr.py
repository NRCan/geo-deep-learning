...
# converted to dict with: https://github.com/rbgirshick/yacs/issues/19
config = {
    "MODEL": {
        "ALIGN_CORNERS": True,
        "EXTRA": {
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, num_bands, out_channels, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(num_bands, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        ...
        self.aux_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(last_inp_channels, out_channels,
    ...
    def forward(self, x):
        ...
        out = F.interpolate(out, size=(x0_h*4, x0_w*4),
                        mode='bilinear', align_corners=ALIGN_CORNERS)

        out_aux_seg.append(out_aux)
        out_aux_seg.append(out)

        return out  # out_aux_seg
...
def get_seg_model(num_bands, num_channels, pretrained, **kwargs):
    model = HighResolutionNet(num_bands, num_channels, **kwargs)
