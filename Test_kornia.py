...
class GeoInference:
    ...
    def __call__(
        ...
        work_dir = "/gpfs/fs5/nrcan/nrcan_geobase/work/dev/datacube/parallel/geo_inference_big_enhancement/"
        ...
        for aoi_dict in data_tiff_paths:
            ...
            data = da.pad(
                ...
                (1, int(aoi_dask_array.shape[1] / 2), int(aoi_dask_array.shape[2] / 2))
            )  # now we rechunk data so that each chunk has 3 bands
            print(aoi.high_or_low_contrast)
            data = da.map_overlap(
                equalize_adapthist_enhancement,
                data,
                data_bool=aoi.high_or_low_contrast,
                clip_limit=25,
                depth={
                    1: patch_size * 2,
                    2: patch_size * 2,
