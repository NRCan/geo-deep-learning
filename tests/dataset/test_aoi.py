...
class Test_AOI(object):
    ...
    def test_stac_input(self):
        ...
        for band, actual_raster in zip(["R", "G", "B"], aoi.raster_parsed):
            ...
            exp_raster = (
                root_raster
                + f"SpaceNet_AOI_2_Las_Vegas-056155973080_01_P001-WV03-{band}.tif"
            )
    ...
    def test_is_low_contrast(self):
        ...
        low_contrast_image = np.random.randint(
            100, 156, size=raster_shape, dtype=np.uint8
        )
    ...
    def test_equalize_hist_raster(self):
        ...
        for per_band in (True, False):
            equ_fp = aoi.equalize_hist_raster(
                clip_limit=125, overwrite=True, per_band=per_band
            )
