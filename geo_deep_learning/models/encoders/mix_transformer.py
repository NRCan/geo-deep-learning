...
def get_encoder(name, in_channels=3, depth=5, weights=None, output_stride=32, **kwargs):
    try:
        ...
        raise KeyError("Wrong encoder name `{}`, supported encoders: {}".format(name, list(mix_transformer_encoders.keys())))
    ...
    if weights is not None:
        if in_channels == 3:
            try:
                settings = mix_transformer_encoders[name]["pretrained_settings"][weights]
            except KeyError:
                raise KeyError(
                    "Wrong pretrained weights `{}` for encoder `{}`. Available options are: {}".format(
                        weights,
                        name,
                        list(mix_transformer_encoders[name]["pretrained_settings"].keys()),
                    )
                )
            encoder.load_state_dict(model_zoo.load_url(settings["url"]))
        else:
            warnings.warn("MixVisionTransformer encoder does not support pretrained weights for non-RGB input channels")
